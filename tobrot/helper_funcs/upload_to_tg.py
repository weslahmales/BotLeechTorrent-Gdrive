    ...
    if os.path.isdir(local_file_name):
        ...
        if os.path.getsize(local_file_name) > TG_MAX_FILE_SIZE:
            ...
            i_m_s_g = await message.reply_text(
                "Telegram tidak mendukung pengunggahan file ini.\n"
                f"Ukuran File Terdeteksi: {d_f_s} 😡\n"
                "\n🤖 Mencoba untuk membagi file 😬"
            )
            ...
            await i_m_s_g.edit_text(
                f"Ukuran File Terdeteksi: {d_f_s} 😡\n"
                f"<code>{ba_se_file_name}</code> dipecah menjadi {number_of_files} files.\n"
                "Mencoba untuk mengunggah ke Telegram, sekarang ..."
            )
...
async def upload_to_gdrive(file_upload, message, messa_ge, g_id):
    ...
    if os.path.isfile(file_upload):
        ...
        await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
        await messa_ge.reply_text(f"🤖: {file_upload} telah berhasil diunggah ke cloud Anda <a href='tg://user?id={g_id}'>😬</a>\n📀 Size: {gjay}", reply_markup=button_markup)
        #await message.edit_text(f"""🤖: {file_upload} telah berhasil diunggah ke cloud Anda 🤗\n\n☁️ Cloud URL:  <a href="{gau_link}">FileLink</a>\nℹ️ Direct URL:  <a href="{tam_link}">IndexLink</a>""")
        ...
        button_markup = pyrogram.InlineKeyboardMarkup(button)
        await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
        await messa_ge.reply_text(f"🤖: Folder berhasil diunggah ke {tt} di Cloud Anda <a href='tg://user?id={g_id}'>🤒</a>\n📀 Size: {gjay}", reply_markup=button_markup)
        #await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
        #await messa_ge.reply_text(f"""🤖: Folder berhasil diunggah ke {tt} di Cloud Anda 🤗\n\n☁️ Cloud URL:  <a href="{gau_link}">FolderLink</a>\nℹ️ Index Url:. <a href="{tam_link}">IndexLink</a>""")
