...
async def extract_youtube_dl_formats(url, cf_name, yt_dl_user_name, yt_dl_pass_word, user_working_dir):
    ...
    if e_response:
        ...
        error_message = e_response.replace(
            "tolong laporkan masalah ini pada https://yt-dl.org/bug . Make sure you are using the latest version; see  https://yt-dl.org/update  on how to update. Be sure to call youtube-dl with the --verbose flag and include its complete output.", ""
        )
    ...
    if t_response:
        ...
        if cf_name:
            succss_mesg = f"""Pilih format yang diinginkan🤗 | {cf_name}"""
        else:
            succss_mesg = f"""Pilih format yang diinginkan🤗"""
